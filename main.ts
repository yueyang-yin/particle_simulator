...
interface GestureTypingOptions {
  onDone?: () => void;
  lineDelayMs?: (lineIndex: number) => number;
}

declare const drawConnectors: (
...
declare const FACEMESH_LIPS: unknown;
declare const FACEMESH_FACE_OVAL: unknown;
...
const enableBtn = document.getElementById("enableCamera") as HTMLButtonElement;
...
const runtimeUi = document.getElementById("runtimeUi") as HTMLDivElement | null;
const modeLine = document.getElementById("modeLine") as HTMLDivElement | null;
const shortcutLine = document.getElementById("shortcutLine") as HTMLDivElement | null;
const terminalEl = document.getElementById("terminal") as HTMLDivElement | null;
const gestureTerminal = document.getElementById("gestureTerminal") as HTMLDivElement | null;
...
let canvasSize: CanvasSize = { width: 0, height: 0, offsetX: 0, offsetY: 0, scale: 1 };
let lastFace: NormalizedLandmark[] | null = null;
let lastHands: NormalizedLandmark[][] | null = null;
let faceReady = false;
...
let apologyTimer: number | null = null;
...
function mapToCanvas(point: NormalizedLandmark): { x: number; y: number } {
...
function isFist(landmarks: NormalizedLandmark[] | undefined): boolean {
...
function maybeSwitchTheme(handsLandmarks: NormalizedLandmark[][] | null): void {
...
function addJitteredPoint(targets: Target[], point: { x: number; y: number }, radius: number, weight = 1, size = 1.2): void {
...
function buildTargets(): { faceTargets: Target[]; handTargets: Target[] } {
  const faceTargets: Target[] = [];
  const handTargets: Target[] = [];
    ...
    lastHands.forEach((hand) => {
      fingerChains.forEach((chain) => {
        for (let i = 0; i < chain.length - 1; i += 1) {
          ...
          const isTip =
            chain[i + 1] === 4 || chain[i + 1] === 8 || chain[i + 1] === 12 || chain[i + 1] === 16 || chain[i + 1] === 20;
...
function updateParticles(faceTargets: Target[], handTargets: Target[]): void {
...
function drawOverlay(resultsFace: FaceMeshResults | null, resultsHands: HandsResults | null): void {
...
function startTyping(): void {
  ...
  const typeStep = () => {
    ...
    if (typingChar < line.length) {
      ...
      typingTimer = window.setTimeout(typeStep, delay);
      return;
    }
    cursor.before("\n");
    typingLine += 1;
    typingChar = 0;
    typingTimer = window.setTimeout(typeStep, 260 + Math.random() * 120);
  };
  typeStep();
}

function showGestureTerminal(): void {
  if (!gestureTerminal) return;
  gestureTerminal.classList.remove("hidden");
}

function clearGestureTerminal(): void {
  if (!gestureTerminal) return;
...
function typeGestureLines(lines: string[], options: GestureTypingOptions = {}): void {
  ...
  const typeStep = () => {
    ...
    if (charIndex < line.length) {
      ...
      gestureTimer = window.setTimeout(typeStep, delay);
      return;
    }
    cursor.before("\n");
    lineIndex += 1;
    ...
    gestureTimer = window.setTimeout(typeStep, pause);
  };
  typeStep();
}

function setShortcutLine(showSpace: boolean): void {
...
function triggerPeaceGesture(): void {
  const now = performance.now();
...
function triggerMiddleGesture(): void {
  ...
  clearGestureTerminal();
  if (apologyTimer) window.clearTimeout(apologyTimer);
...
function endMiddlePunishment(): void {
  if (!middleActive) return;
  middleActive = false;
  apologyShown = false;
  mode = "attract";
...
function fingerExtended(hand: NormalizedLandmark[], tipIdx: number, pipIdx: number, wristIdx = 0): boolean {
...
function fingerCurled(hand: NormalizedLandmark[], tipIdx: number, pipIdx: number, wristIdx = 0): boolean {
  const tip = hand[tipIdx];
  const pip = hand[pipIdx];
  const wrist = hand[wristIdx];
  const tipDist = Math.hypot(tip.x - wrist.x, tip.y - wrist.y);
...
function isPeaceGesture(hand: NormalizedLandmark[]): boolean {
...
function isMiddleFingerGesture(hand: NormalizedLandmark[]): boolean {
  const middleExt = fingerExtended(hand, 12, 10);
...
function handleGestures(hands: NormalizedLandmark[][] | null): void {
...
function startManualFrameLoop(faceMesh: FaceMesh, hands: Hands): void {
  const step = async () => {
    try {
